...
create_issue() {
  ...
  local existing
  existing="$(gh issue list --repo "$repo" --state all --search "$id in:title" --json number --jq '.[0].number // empty')"
  if [[ -n "$existing" ]]; then
...
"agent:ready" "area:frontend" "area:api-client" "type:tech-debt"
...
"agent:ready" "area:frontend" "area:compliance" "sprint:legal-1" "risk:legal"
...
"agent:ready" "area:frontend" "area:auth" "area:compliance" "sprint:legal-1" "risk:legal"
...
create_issue "[FE-REPORT-001] Ajouter un composant réutilisable de formulaire de signalement" \
...
create_issue "[FE-REPORT-002] Ajouter les actions de signalement sur profil et annonces" \
...
create_issue "[FE-ADMIN-001] Ajouter la page admin de modération" \
...
"blocked:backend-contract" "area:frontend" "area:admin" "area:compliance" "sprint:legal-1" "risk:legal"

create_issue "[FE-ADMIN-002] Ajouter les actions UI de modération" \
...
"blocked:backend-contract" "area:frontend" "area:auth" "area:compliance" "sprint:legal-1" "risk:legal"
...
"agent:needs-review" "area:frontend" "sprint:ops" "type:tech-debt"
