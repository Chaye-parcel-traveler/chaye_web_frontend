...
export const useAccountRestrictions = () => {
  const [accountStatus, setAccountStatus] = useState<AccountStatusResponse>(
    () => getLocalAccountStatus()
  );

  useEffect(() => {
    if (getLocalAccountStatus().status !== defaultStatus.status) {
      return;
    }
...
const getLocalAccountStatus = (): AccountStatusResponse => {
  ...
  return {
    status: localStatus,
    reason: window.localStorage.getItem('chaye_account_status_reason') ?? '',
  };
};
