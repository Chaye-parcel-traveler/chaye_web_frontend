...
- starts MariaDB as a CI service;
- starts the API on `http://127.0.0.1:3333`;
- builds the frontend with `VITE_API_URL=http://127.0.0.1:3333`;
- starts the frontend on `http://127.0.0.1:3000`;
- runs Playwright;
- uploads the Playwright report and service logs when the job fails.
